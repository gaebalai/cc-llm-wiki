...
- 2026-05-18T11:30:00+09:00 | [release] | cc-session | v0.4.3: daily-digest 본체 (positioning + WebSearch 호출 명세). 사용자 ~/my-knowledge-base/positioning.md 자동 활성 (markdown 코드 블록 → 실 frontmatter, 백업 .template-backup). daily_digest_runner.py 보강: digest 페이지 골격 출력 (Skill 이 WebSearch 결과로 채울 자리). SKILL.md WebSearch+WebFetch 호출 형식 + URL 중복 배제 명령 명확화. CLAUDE.md §11 daily-digest: dry-run → ✅ active | scripts/daily_digest_runner.py skills/daily-digest/SKILL.md CLAUDE.md ~/my-knowledge-base/positioning.md
